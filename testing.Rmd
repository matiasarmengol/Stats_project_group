...
vars <- c('Y'='demand_gross', 'Date', 'Wind'='wind', 'Solar'='solar_S', 'Temp'='temp', 'Day_Index'='wdayindex', 'Month_Index'='monthindex', 'TE', 'TO')
...
SCS_hourly_temp$Date <- as.POSIXct(SCS_hourly_temp$Date)
...
vars2 <- c('wind', 'solar_S', 'Y' = 'demand_gross', 'temp', 'wdayindex', 'monthindex', 'year', 'TO', 'TE', 'start_year', 'DSN', 'daily_avg', 'daily_var')
...
selected$season <- factor(ifelse(selected$Month_Index %in% c(0, 1, 10, 11), 1, 0), 
                    levels = c(0, 1), 
                    labels = c("Not Winter", "Winter"))

Model_M3 <- lm(Y ~ Solar + Temp + week_type + season, data = selected)
