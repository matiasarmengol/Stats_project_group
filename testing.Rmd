...
SCS_hourly_temp$Date <- as.POSIXct(SCS_hourly_temp$Date, format = "%d/%m/%Y %H:%M:%S",
    daily_avg = mean(temp, na.rm = TRUE),
    daily_var = var(temp, na.rm = TRUE)
  )

big_set <- SCS_hourly_temp
  
big_set <- big_set |> 
  mutate(Date = as.POSIXct(big_set$Date, format = "%d/%m/%Y %H:%M:%S")) |> 
  left_join(SCS_demand_modelling, by = "Date")
