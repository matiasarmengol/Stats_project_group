...
results_all <- cross_validate_models(models, big_set)
```
...
comparisons_df <- mutate(comparisons_df, diff = abs(control_max - max_predicted))
...
ggplot(data = comparisons_df) +
  geom_point(aes(x = other_year, y = diff)) + 
  geom_smooth(aes(x = other_year, y = diff)) +
  ggtitle("Predicted Difference If 2013/14 Had Another Winter's Weather")
...
models <- list(Model_M0, Model_M5)

cross_validate_models(models)
```
