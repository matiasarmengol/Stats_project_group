...
Below we have a plot of the average demand against year and against month respectively.
...
```
<div style="height: 10px;"></div>
From the yearly plot we observe a strong relationship between the year and the demand. Were we to plot Average Gross Demand as solely a function of year, we would conjecture some cyclical or polynomial relationship. However, the plot we observe here is sufficient for us to confidently include year as a coefficient in our candidate models going forwards.
```{r demand_vs_month, echo=FALSE, message=FALSE, fig.align = 'center', fig.cap = 'Figure 3: Bar plot of average monthly gross demand against month, for the months of November, December, January, February and March. This highlights the significant different between March and the other months.'}
...
```
<div style="height: 10px;"></div>
As the number of data points on the month scale was sufficiently low, we chose to use a bar plot instead of treating the time scale as continuous. 

The most visible result here is the difference between the month of March and the other months. Qualitatively we can expect this, as March would be expected to be different in terms of weather to the other months for which we have data. It is reasonable to expect this to translate into the demand during this time.

```{r temp_demand, echo=FALSE, message=FALSE, fig.align = 'center', fig.cap = 'Figure 4: Scatter plot of gross demand against population weighted average tmeperature, showing a significant negative correlation.'}
...
```
<div style="height: 10px;"></div>
A reasonable assumption would be to expect energy demand to decrease with temperature, as the energy required to heat homes is reduced. However, this plot sees a very weak negative correlation between the two. This implies that the variation in gross demand has very little dependency on temperature.

However, through observation of the correlation matrix as well as comparison of model results with versus without the temperature variable, we have decided to keep it.
...
M0_cv <- k_fold_cross_validate(M0, df, strat_vars = c("is_weekday", "is_winter"))
M5_cv <- k_fold_cross_validate(M5, df, strat_vars = c("is_weekday", "is_winter"))
...
metrics_M0 <- data.frame(
  model = "M0",
  metric = names(M0_cv$Overall$overall_metrics),
  value = unlist(M0_cv$Overall$overall_metrics)
)

metrics_M5 <- data.frame(
  model = "M5",
  metric = names(M5_cv$Overall$overall_metrics),
  value = unlist(M5_cv$Overall$overall_metrics)
)

compare_set <- rbind(metrics_M0, metrics_M5)

# Plot
ggplot(compare_set, aes(x = metric, y = value, fill = model)) +
...
```{r compare_winter_plot_1, echo=FALSE, message=FALSE, fig.align = 'center', fig.cap = 'Figure 5: '}
...
```
<div style="height: 10px;"></div>
```{r compare_winter_plot_2, echo=FALSE, message=FALSE, fig.align = 'center', fig.cap = 'Figure 6: '}
